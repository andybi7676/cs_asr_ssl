...
def get_costum_scheduler(
    optimizer: Optimizer, num_training_steps: int, num_warmup_steps: int, num_decay_steps: int, last_epoch: int = -1
):
    """
        ________
       /        \\
    ...
    def lr_lambda(current_step):
        ...
        if current_step < num_training_steps - num_decay_steps:
            return float(1.0)
        return max(0.0, 1.0-(num_training_steps-current_step)/(num_training_steps-num_decay_steps))
